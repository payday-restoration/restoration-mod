function IngameContractGui:init(ws, node)
	...
	if job_data then
		...
		self._cash_title = cash_title
		self._experience_title = experience_title
		self._stage_cash_title = stage_cash_title
...
function IngameContractGui:set_potential_rewards(show_max)
	...
	local stage_cash_title = self._stage_cash_title
	local cash_title = self._cash_title
	local sx = math.max(stage_cash_title:right(), cash_title:right()) + 8
	...
	if show_max then
		...
		total_payout, stage_payout_table, job_payout_table = managers.money:get_contract_money_by_stars(job_stars, difficulty_stars, #job_chain, job_id, nil, {
		...
		total_payout, stage_payout_table, job_payout_table = managers.money:get_contract_money_by_stars(job_stars, difficulty_stars, #job_chain, job_id)
	end
	...
	local cy = stage_cash_title:center_y()
	local stage_cash = self._rewards_panel:text({
		font = tweak_data.menu.pd2_small_font,
		font_size = font_size,
	...
	local stage_add_cash = self._rewards_panel:text({
		font = tweak_data.menu.pd2_small_font,
		font_size = font_size,
		text = "",
		color = risk_color
