...
function ModifierEnemyHealthAndDamageByWave:get_health_multiplier()
	local current_wave = math.max(managers.skirmish:current_wave_number(), 1)

	return self._waves[current_wave] and self._waves[current_wave].health or 1
end

function ModifierEnemyHealthAndDamageByWave:get_damage_multiplier()
	local current_wave = math.max(managers.skirmish:current_wave_number(), 1)

	return self._waves[current_wave] and self._waves[current_wave].damage or 1
end
