...
function ObjectiveBox_create( panel, params, config )
...
function ObjectiveBox_animate_open_right( panel, wait_t, target_w, done_cb )
	...
	bg:stop()
	bg:animate( callback( nil, _G, "ObjectiveBox_animate_bg_attention" ) )
...
function ObjectiveBox_animate_close_right( panel, done_cb )
	local speed = box_speed
	local cw = panel:w()
...
function ObjectiveBox_animate_bg_attention( bg, config )
...
function HUDObjectives:init( hud )
	...
	self._bg_box = ObjectiveBox_create( objectives_panel, { w = 200, h = 38, x = 0, y = 0 }, {color = Color(0, 0, 0, 0)} ) -- HUDBGBox:new( objectives_panel, { w = 200, h = 40, y = 20 } )
	
	local bar_left = self._bg_box:bitmap( { halign="left", valign="top", name = "bar_left", blend_mode = "normal", color = Color( 80/255, 185/255, 240/255 ), visible = true, layer = 0, texture = "units/world/architecture/desert/des_bazaar_shop/des_bazaar_shop_df", x = 0, y = 0, w = 8, h = 35 } )
	local bar_mid = self._bg_box:bitmap( { halign="left", valign="top", name = "bar_mid", blend_mode = "normal", color = Color( 80/255, 185/255, 240/255 ), visible = true, layer = 0, texture = "units/world/architecture/desert/des_bazaar_shop/des_bazaar_shop_nm", x = 0, y = 0, w = 32, h = 35 } )
	local bar_right = self._bg_box:bitmap( { halign="left", valign="top", name = "bar_right", blend_mode = "normal", color = Color( 80/255, 185/255, 240/255 ), visible = true, layer = 0, texture = "units/world/architecture/desert/des_bazaar_stand/des_bazaar_stand_df", x = 0, y = 0, w = 8, h = 35 } )
	...
	local amount_text = objectives_panel:text( { name = "amount_text", visible = true, layer = 2, color = Color( 0, 0, 0 ), text="1/4", font_size = 22, font = tweak_data.hud.medium_font_noshadow, x = 6, y = 0, align = "left", vertical = "top" } )
...
function HUDObjectives:activate_objective( data )
	...
	self._bg_box:stop()
	self._bg_box:animate( callback( nil, _G, "ObjectiveBox_animate_open_right" ), 0.66, w + 8*2 + w2 + 8, callback( self, self, "open_right_done", data.amount and true or false ) )
...
function HUDObjectives:_animate_complete_objective( objectives_panel )
	...
	-- Close box
	self._bg_box:stop()
	self._bg_box:animate( callback( nil, _G, "ObjectiveBox_animate_close_right" ), done_cb )
end
