...
HUDAssaultCorner._custom_lines_casing = {
        ...
        "CREDITS  ///  LEGO, CGNICK, KAIL, SHATTER, SUPER MUFFIN, WOLFY, BUSHY, OLIPRO, INSANO, MELONIOUS, LAZYOZZY, PORKY-DA-CORGI, DOKTOR AKCEL, AND A.J. VALENTINE  ///  THANK YOU EVERYONE <3",
        "CRIME.NET ACTUALLY HAS ADWARE, THANKS BAIN - WOLF",
        "CAN WE SPEND OFFSHORE CASH ON CIGARS YET? - DALLAS",
        "WHY ARE PEOPLE ALWAYS SAYING 'WANKER' AROUND ME? - HOXTON",
        "AT LEAST CRIME.NET DOESN'T JAM - CHAINS",
        "SHIT! - HOUSTON, PROBABLY",
        "HEY FUCK YOU HOXTON - HOUSTON FOR REAL",
        "COULD BE GREENER... - CLOVER",
        "ONE! - WICK, PROBABLY",
        "SORRY, I HIT THE KEYBOARD WITH MY BONER - DRAGAN",
        "I DO LIKE HURTING OTHER PEOPLE! - JACKET",
...
HUDAssaultCorner._custom_lines_assault = {
        ...
        "SORRY... CAN'T REPLACE SKULLS ON THE TAPE.  MAYBE IN A HOTFIX LOL",
...
HUDAssaultCorner._custom_line_chance = 5 -- Chance of a new line, between 0-100
...
function HUDAssaultCorner:flash_assault(o)
	while true do
		local a = 0.25 + (math.sin(Application:time() * 625) + 1) / 4
		o:set_alpha(a)
		o:set_color(Color(a, 0, 0.8 - a))
		coroutine.yield()
	end
end

function HUDAssaultCorner:flash_assault_tape(o)
	while true do
		o:set_color( o:color():with_alpha( 0.5 + (math.sin( Application:time()*625 )+1)/4 ) ) --0.5 is default
		coroutine.yield()
	end
end
...
function HUDAssaultCorner:init(hud, full_hud)
	...
	self._assault_color_2 = Color.blue--Color(0.25, 0, 0.55)
...
function HUDAssaultCorner:_start_assault(text_list)
    ...
    local corner = assault_panel:child("corner")
    local corner2 = assault_panel:child("corner2")
    local assault_text = assault_panel:child("assault_text")
...
function HUDAssaultCorner:_end_assault()
	...
	self._assault = false
	--local assault_tape = self._hud_panel:child("assault_panel"):child("assault_tape")
	--assault_tape:set_visible(false)
	
	local assault_panel = self._hud_panel:child("assault_panel")
    local corner = self._hud_panel:child("assault_panel"):child("corner")
    local corner2 = self._hud_panel:child("assault_panel"):child("corner2")
