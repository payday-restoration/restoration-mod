...
if Restoration.options.restoration_presenter_global == true then
...
function HUDPresenter:init( hud )
	...
	local title = message_panel:text( { name = "title", text = "SIZE", vertical="bottom", valign="grow",  layer = 1, color = (Color.white/2):with_alpha( 1 ), font = "fonts/font_medium_noshadow_mf", font_size = 25 } )
	local _,_,_,h = title:text_rect()
	title:set_h( h )
	title:set_bottom( math.floor( message_panel:h()/ 2 ) + 2 )
	
	local text = message_panel:text( { name = "text", text = "SIZE", vertical="top", valign="grow", layer = 1, color = Color.white, font = "fonts/font_medium_noshadow_mf", font_size = tweak_data.hud_present.text_size } )
	local _,_,_,h = text:text_rect()
	text:set_h( h )
