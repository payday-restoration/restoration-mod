		...
		for i, hit in ipairs(ray_hits) do
			if not units_hit[hit.unit:key()] then
				...
				elseif not self._can_shoot_through_shield and hit.unit:in_slot(shield_mask) then
					break
				elseif hit.unit:in_slot(shield_mask) and hit.unit:name():key() == 'af254947f0288a6c' and self._name_id ~= "m95" and self._name_id ~= "rpg7" and self._name_id ~= "ray" and self._name_id ~= "ks23" and self._name_id ~= "lynx" and self._name_id ~= "inkunzi" then
					break
