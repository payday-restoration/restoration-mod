NPCBowWeaponBase = NPCBowWeaponBase or class(BowWeaponBase)
...
NPCCrossBowWeaponBase = NPCCrossBowWeaponBase or class(CrossbowWeaponBase)

function NPCCrossBowWeaponBase:fire_blank(direction, impact)
	self:_sound_singleshot()
	if self:weapon_tweak_data().has_fire_animation then
		self:tweak_data_anim_play("fire")
	end
