		...
		if can_dodge then --Dodge attacks if your meter is at '100'.
			...
			if attack_data.damage > 0 
				...
				if unit_movement then
					...
					local advmov = current_state and (current_state._is_sliding or current_state._is_wallrunning)
					...
					if crouched or freefall or unit_movement:zipline_unit() or current_state.driving or current_state._moving ~= true then
						drain_mult = 0
					elseif (unit_movement:running() or current_state:in_air() or advmov) then
						drain_mult = 1
					end
				end
