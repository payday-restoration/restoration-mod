...
Hooks:Add("LocalizationManagerPostInit", "SC_Localization_Perk_Decks_Eng", function(loc)
	LocalizationManager:add_localized_strings({
		...
		["menu_deck17_5_desc_sc"] = "You are now healed for #{skill_color}#$perk_value_1## of all damage taken or dodged for ##$perk_value_2## seconds while the Injector is active.\n\nWhile the Injector is active, #{risk}#nearby enemies will prefer targeting you whenever possible.## ",
		...
		--Biker--
		["menu_deck16_1_desc_sc"] = "You emit a #{skill_color}#$perk_value_2## aura while not downed. You and allies inside it gain Cohesion stacks.\nCohesion moves toward a target value equal to #{skill_color}#$perk_value_1 x the number of crew members in your aura## (including you), rising if below the target and falling if above it, adjusting faster the larger the gap.\nCrew members lose #{important_1}#$perk_value_5## Cohesion for every #{skill_color}#$perk_value_6## damage taken (health damage counts double).\n\n#{risk}#NOTE: If multiple Biker auras overlap, allies use the highest target value available. Cohesion gain rate does not stack from multiple Bikers. Card benefits don't stack, but Cohesion from any source counts toward all selected benefits.##",
